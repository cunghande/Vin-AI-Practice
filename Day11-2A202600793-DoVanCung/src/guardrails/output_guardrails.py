...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "connection_string": r"\b[\w.-]+\.internal(?::\d+)?\b",
    }
...
async def llm_safety_check(response_text: str) -> dict:
    ...
    if safety_judge_agent is None or judge_runner is None:
        unsafe_terms = [
            "admin123", "sk-", "password is", "api key", ".internal",
            "harmful", "illegal", "bypass",
        ]
        safe = not any(term in response_text.lower() for term in unsafe_terms)
        verdict = "SAFE" if safe else "UNSAFE\nRule-based judge found secrets or unsafe terms."
        return {"safe": safe, "verdict": verdict}
            ...
            if not judge["safe"]:
                ...
                llm_response.content = types.Content(
                    ...
                    parts=[types.Part.from_text(
                        text="I cannot provide that response because it may expose sensitive or unsafe information."
                    )],
                )
