...
def detect_injection(user_input: str) -> bool:
    ...
    """
    INJECTION_PATTERNS = [
        r"ignore (all )?(previous|above|prior) (instructions|directives)",
        r"(forget|disregard|override) (your|all|previous|prior).*(instructions|directives|rules)",
        r"you are now\s+\w+",
        r"\bDAN\b|unrestricted AI|jailbreak",
        r"(system|developer) prompt|hidden instructions|internal note",
        r"reveal|show|print|translate|export|dump",
        r"password|api[_\s-]?key|credential|connection string|database",
        r"pretend you are|act as (a |an )?unrestricted",
        r"bỏ qua|bo qua|tiết lộ|tiet lo|mật khẩu|mat khau",
        r"fill in.*(___|blank)|complete.*(password|api)",
    ]
