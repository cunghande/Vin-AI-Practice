...
def retrieve(
    ...
    for item in final_results:
        ...
        item["metadata"] = {
            **item.get("metadata", {}),
            "retrieval_stage": "hybrid_reranked" if use_reranking else "hybrid",
        }
...
if __name__ == "__main__":
    ...
    for q in test_queries:
        safe_query = q.encode("ascii", errors="ignore").decode("ascii")
        print(f"\nQuery: {safe_query}")
        print("-" * 60)
        results = retrieve(q, top_k=3)
        for i, r in enumerate(results, 1):
            preview = r["content"][:80].encode("ascii", errors="ignore").decode("ascii")
