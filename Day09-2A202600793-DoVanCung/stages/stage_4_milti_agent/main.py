...
async def check_routing(state: LegalState) -> dict:
    ...
    needs_tax = any(kw in question_lower for kw in ["tax", "irs", "revenue", "offshore", "thuế"])
    needs_compliance = any(
        kw in question_lower
        for kw in ["compliance", "sec", "regulation", "sox", "fcpa", "aml"]
    )
    needs_privacy = any(
        kw in question_lower
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy specialist sub-agent for GDPR, CCPA, and data protection issues."""
    print("\n  [Node: call_privacy_specialist] Privacy specialist agent starting...")

    privacy_prompt = (
        "You are a privacy attorney specialising in GDPR, CCPA/CPRA, consent, data "
        "protection, privacy rights, breach notification, and data governance. Analyse "
        "privacy and data protection issues in the user's question. Keep your response "
        "under 200 words."
    )
...
QUESTION = (
    "If a company leaks customer data and avoids taxes on overseas revenue, what are "
    "the legal, privacy, tax, and regulatory consequences?"
)
