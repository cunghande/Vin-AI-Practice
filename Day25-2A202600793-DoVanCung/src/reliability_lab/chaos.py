...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            ...
            elif state_to == "closed" and open_ts is not None:
                close_ts = float(entry["ts"])
                recovery_times.append((close_ts - open_ts) * 1000)
                open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    gateway = build_gateway(config, scenario.provider_overrides if scenario.provider_overrides else None)
    metrics = RunMetrics()
    ...
    # Count circuit opens from breaker logs
    open_count = 0
    for breaker in gateway.breakers.values():
        for entry in breaker.transition_log:
    ...
    if gateway.cache is not None and hasattr(gateway.cache, "close"):
        gateway.cache.close()
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        elif scenario.name == "both_failed_100":
            passed = result.static_fallbacks == result.total_requests
        else:
