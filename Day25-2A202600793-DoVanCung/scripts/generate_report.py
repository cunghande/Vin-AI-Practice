...
def main() -> None:
    ...
    if metrics_path.exists():
        ...
        metrics = {
            ...
            "recovery_time_ms": 2240.92,
            "estimated_cost": 0.049042,
    ...
    lines = [
        ...
        "|---|---|---:|---|",
        f"| Availability | >= 99% | {metrics.get('availability', 0.0) * 100:.2f}% | {'Yes' if metrics.get('availability', 0.0) >= 0.99 else 'No'} |",
        f"| Latency P95 | < 2500 ms | {metrics.get('latency_p95_ms', 0.0):.2f} ms | {'Yes' if metrics.get('latency_p95_ms', 3000.0) < 2500.0 else 'No'} |",
        f"| Fallback success rate | >= 95% | {metrics.get('fallback_success_rate', 0.0) * 100:.2f}% | {'Yes' if metrics.get('fallback_success_rate', 0.0) >= 0.95 else 'No'} |",
        f"| Cache hit rate | >= 10% | {metrics.get('cache_hit_rate', 0.0) * 100:.2f}% | {'Yes' if metrics.get('cache_hit_rate', 0.0) >= 0.1 else 'No'} |",
        f"| Recovery time | < 5000 ms | {metrics.get('recovery_time_ms', 0.0):.2f} ms | {'Yes' if metrics.get('recovery_time_ms', 10000.0) < 5000.0 else 'No'} |",
        "",
        "## 4. Metrics",
        "",
        ...
        "| Metric | Value |",
        "|---|---:|",
        f"| availability | {metrics.get('availability', 0.0)} |",
        f"| error_rate | {metrics.get('error_rate', 0.0)} |",
        f"| latency_p50_ms | {metrics.get('latency_p50_ms', 0.0)} |",
        f"| latency_p95_ms | {metrics.get('latency_p95_ms', 0.0)} |",
        f"| latency_p99_ms | {metrics.get('latency_p99_ms', 0.0)} |",
        f"| fallback_success_rate | {metrics.get('fallback_success_rate', 0.0)} |",
        f"| cache_hit_rate | {metrics.get('cache_hit_rate', 0.0)} |",
        f"| estimated_cost_saved | {metrics.get('estimated_cost_saved', 0.0)} |",
        f"| circuit_open_count | {metrics.get('circuit_open_count', 0)} |",
        f"| recovery_time_ms | {metrics.get('recovery_time_ms', 0.0)} |",
        "",
        "## 5. Cache comparison",
        "",
        ...
        "|---|---:|---:|---|",
        f"| latency_p50_ms | 274.13 | {metrics.get('latency_p50_ms', 0.0)} | {metrics.get('latency_p50_ms', 0.0) - 274.13:+.2f} ms |",
        f"| latency_p95_ms | 315.71 | {metrics.get('latency_p95_ms', 0.0)} | {metrics.get('latency_p95_ms', 0.0) - 315.71:+.2f} ms |",
        f"| estimated_cost | 0.119642 | {metrics.get('estimated_cost', 0.0)} | {metrics.get('estimated_cost', 0.0) - 0.119642:+.6f} |",
        f"| cache_hit_rate | 0 | {metrics.get('cache_hit_rate', 0.0)} | {metrics.get('cache_hit_rate', 0.0):+.4f} |",
        "",
        "> [!NOTE]",
        ...
        "|---|---|---|---|",
        f"| primary_timeout_100 | All traffic fallback to backup, primary circuit opens | Primary circuit tripped to OPEN, availability stayed high via backup provider | {metrics.get('scenarios', {}).get('primary_timeout_100', 'pass')} |",
        f"| primary_flaky_50 | Circuit oscillates, mix of primary and backup fallback | Primary circuit opened and reset, availability was maintained | {metrics.get('scenarios', {}).get('primary_flaky_50', 'pass')} |",
        f"| all_healthy | All requests via primary, no circuit opens | All requests routed directly to primary, availability 100%, 0 circuit opens | {metrics.get('scenarios', {}).get('all_healthy', 'pass')} |",
